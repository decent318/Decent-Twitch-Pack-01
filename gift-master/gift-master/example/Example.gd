...
@export var client_id : String = "mkazvb1k08wuw5yykemvai5qjc019b"
...
func hello(cmd_infomation : CommandInfo) -> void:
...
func join_game(cmd_info : CommandInfo) -> void:
	if joined.find(cmd_info.sender_data.user) == -1:
		irc.chat("Joined!")
		joined.append(cmd_info.sender_data.user)
	else:
		irc.chat("You are already joined!")
	irc.chat(str(joined))
	
func leave_game(cmd_info : CommandInfo) -> void:
	irc.chat("Left!")
	joined.erase(cmd_info.sender_data.tags["display-name"])
	irc.chat(str(joined))
